...
var Server = {
    ...
    run: function (paraConfig, route) {
        ...
        // 获取配置文件
        var notCheckDomainPage = [];
        var st = null;
        try {
            var shopConfig = JSON.parse(Fs.readFileSync(this.config.configFile).toString());
            st = shopConfig['st'];
            host = shopConfig['host'];
        } catch (e) {
            console.log(e);
        }

        if (buildType) {
            this.build(miniFile, st);
        } else {
            if (host) {
                ...
                console.log('shop.json host not exist');
            }
        }
    },
                ...
                if (stat.isFile()) {
                    ...
                    if (extName.match(fileExt)) {
                        ...
                        if (Fs.existsSync(targetDir)) {
                            ...
                            //} else if(extName == 'css') {
                            //    targetPath = targetDir + '/' + path.replace('.css', '') + '.' + self.version + '.css';
                            //}
    ...
    buildView: function (miniFile) {
        ...
        var self = this;
        var sourceDir = self.config.devDir + '/' + self.config.build.viewDir;
        var targetDir = self.config.build.distDir + '/scripts';
        //var libDir = self.config.output.lib;
        var fromSource = sourceDir;
        var toSource = targetDir;
        var source;
                ...
                if (stat.isDirectory()) {
                    ...
                    if (file) {
                        ...
                        viewScript.push(viewScriptString +
                            self.createViewObject(nodePath + '.' + childs[0], result.join('')) + ';');
                    }
                }
            });
        };
    ...
    build: function (miniFile, st) {
        ...
        // 拷贝
        this.copyDir(distDir, targetDir, fileExt, miniFile);
        // 清空缓存文件夹
        this.delDir(distDir);
     ...
     * @param st
     */
    buildIndex: function (distDir, st) {
        console.log('构建首页...');
        var self = this;
        var createData = function (fileName) {
            ...
            for (var i in scriptPathList) {
                scriptList.push('<script type="text/javascript" src="' + scriptPathList[i] + '?' + self.version + '"></script>\n');
            }
            ...
            for (var i in stylePathList) {
                styleList.push('<link rel="stylesheet" href="' + stylePathList[i] + '?' + self.version + '" />\n');
            }
            ...
            // 替换统计服务参数
            file = file.replace('{{ st }}', JSON.stringify(st));
