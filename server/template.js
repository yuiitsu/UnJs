...
Template.parse_server_html = function (template_file_path, tmp_param ) {
    var pattern = /\{\{\s?([^]*?)(\.([^]*?))?\s?\}\}/i,
        result = null;
    tmp_param = tmp_param || {};
    return new Promise(function(resolve, reject) {
        Fs.readFile(template_file_path, 'utf-8', function (error, file) {
            if (error) {
                console.warn(error);
                reject(error);
                return;
            ...
            while (result = pattern.exec(file)) {
                if (result[2]) {
                    file = file.replace(result[0], tmp_param[result[1]][result[3]]);
                } else {
                    file = file.replace(result[0], tmp_param[result[1]]);
                }
            }
            resolve(file)
        });
